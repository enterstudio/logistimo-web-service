 ...
 */
public class MaterialTemplate implements ITemplate {
  private final IMaterial material;

  public MaterialTemplate(IMaterial material) {
  ...
  public Map<String, String> getTemplateValues(Locale locale, String timezone,
    ...
    if (excludeVars == null || !excludeVars.contains(EventsConfig.VAR_CREATIONTIME)) {
      varMap.put(EventsConfig.VAR_CREATIONTIME, LocalDateUtil.format(material.getTimeStamp(), locale, timezone));
    }
    if (excludeVars == null || !excludeVars.contains(EventsConfig.VAR_MATERIAL)) {
      varMap.put(EventsConfig.VAR_MATERIAL, material.getName());
    }
    ...
    if (material.getCreatedBy() != null && (excludeVars == null || !excludeVars
        .contains(EventsConfig.VAR_REGISTEREDBY))) {
      varMap.put(EventsConfig.VAR_REGISTEREDBY, material.getCreatedBy());
    }
    if(material.getLastUpdatedBy() != null) {
      varMap.put(EventsConfig.VAR_UPDATEDBY, material.getLastUpdatedBy());
    }
    return varMap;
  }
}
